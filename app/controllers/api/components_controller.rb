class Api::ComponentsController < Api::BaseApiController
  ...
  def index
    ...
    if params[:livesearch].present? 
      ...
      @objects = Component.active_objects.where{
        (is_deleted.eq false) & 
        (
          (name =~  livesearch )  
        )
      ...
      @objects = Component.
                  where(:machine_id => params[:parent_id]).
                  page(params[:page]).per(params[:limit]).order("id DESC")
      @total = Component.where(:machine_id => params[:parent_id]).count 
    else
      
      @objects = Component.active_objects.page(params[:page]).per(params[:limit]).order("id DESC")
  ...
  def search
    ...
    if  selected_id.nil?
      @objects = Component.active_objects.where{ (name =~ query)   
                              }.
                        page(params[:page]).
                        per(params[:limit]).
                        order("id DESC")
      ...
      @objects = Component.active_objects.where{ (id.eq selected_id)  
                              }.
                        page(params[:page]).
                        per(params[:limit]).
                        order("id DESC")
